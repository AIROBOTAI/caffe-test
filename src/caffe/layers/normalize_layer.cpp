...
void NormalizeLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  ada_init_ = norm_param.ada_init();
  first_bt_ = 1;
  count_ = 0;
  if (ada_init_ == 1) {
    if (bottom.size() != 2)
      LOG(FATAL) << "Unmatch bottom size";
...
void NormalizeLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  if (ada_init_ == 1 && this->phase_ == TRAIN) {
    if (first_bt_ == 1) {
      caffe_set(this->blobs_[0]->count(), Dtype(0), this->blobs_[0]->mutable_cpu_data());
      const Dtype* ref_data = bottom[1]->cpu_data();
      int ref_count = bottom[1]->count();
      int ref_num = bottom[1]->num();
      int ref_dim = ref_count / ref_num;
      int ref_spatial_dim = bottom[1]->height() * bottom[1]->width();
      int ref_channels = bottom[1]->channels();
      Blob<Dtype> c_norm(1, ref_channels,1, 1);
      Blob<Dtype> c_norm_temp(1, ref_channels, 1, 1);
      Blob<Dtype> spatial_buffer(1, 1, bottom[1]->height(), bottom[1]->width());
      Dtype* c_norm_data = c_norm.mutable_cpu_data();
      Dtype* c_norm_temp_data = c_norm_temp.mutable_cpu_data();
      Dtype* spatial_buffer_data = spatial_buffer.mutable_cpu_data();
      caffe_set<Dtype>(c_norm.count(), Dtype(0), c_norm_data);
      caffe_set<Dtype>(c_norm_temp.count(), Dtype(0), c_norm_temp_data);
      caffe_set<Dtype>(spatial_buffer.count(), Dtype(0), spatial_buffer_data);

      Blob<Dtype> ref_buffer(1, ref_channels, bottom[1]->height(), bottom[1]->width());
      Dtype* ref_buffer_data = ref_buffer.mutable_cpu_data();
      Dtype spatial_sum = 0;

      for (int n = 0; n < ref_num; ++n) {
        caffe_abs<Dtype>(ref_dim, ref_data, ref_buffer_data);
        if (channel_shared_) {
          c_norm_temp_data[0] = caffe_cpu_asum<Dtype>(ref_dim, ref_buffer_data);
          c_norm_data[0] += Dtype(c_norm_temp_data[0] / ref_dim);
        } else {
          c_norm_temp_data[0] = caffe_cpu_asum<Dtype>(ref_dim, ref_buffer_data);
          c_norm_data[0] += Dtype(c_norm_temp_data[0] / ref_dim);

          // vector<Dtype> tst(ref_spatial_dim, 0);
          // for (int c = 0; c < ref_channels; ++c) {
          //   for (int s = 0; s < ref_spatial_dim; ++s) {
          //     int id = c * ref_spatial_dim + s;
          //     tst[s] += ref_buffer_data[id];
          //   }
          // }
          // caffe_cpu_gemv<Dtype>(CblasTrans, ref_channels, ref_spatial_dim, Dtype(1), ref_buffer_data,
          //     sum_channel_multiplier_.cpu_data(), Dtype(0), spatial_buffer_data);
          // for (int s = 0; s < ref_spatial_dim; ++s) {
          //   if (tst[s] != spatial_buffer_data[s])
          //     LOG(FATAL) << "Incorrect gemv: " << tst[s] << " " << spatial_buffer_data[s];
          // }
          // caffe_scal<Dtype>(ref_spatial_dim, Dtype(1.0 / ref_channels), spatial_buffer_data);
          // spatial_sum += caffe_cpu_asum<Dtype>(ref_spatial_dim, spatial_buffer_data);
          // vector<Dtype> tst(ref_channels, 0);
          // for (int c = 0; c < ref_channels; ++c) {
          //   for (int s = 0; s < ref_spatial_dim; ++s) {
          //     int id = c * ref_spatial_dim + s;
          //     tst[c] += ref_buffer_data[id];
          //   }
          // }
          // caffe_cpu_gemv<Dtype>(CblasNoTrans, ref_channels, ref_spatial_dim, Dtype(1), ref_buffer_data,
          //     sum_spatial_multiplier_.cpu_data(), Dtype(0), c_norm_temp_data);

          // for (int c = 0; c < ref_channels; ++c) {
          //   if (tst[c] != c_norm_temp_data[c])
          //     LOG(FATAL) << "Incorrect gemv: " << tst[c] << " " << c_norm_temp_data[c];
          // }
          // caffe_scal<Dtype>(ref_channels, Dtype(1.0 / ref_spatial_dim), c_norm_temp_data);
          // caffe_add<Dtype>(ref_channels, c_norm_temp_data, c_norm_data, c_norm_data);
        }
        ref_data += ref_dim;
      }

      if (channel_shared_) {
        c_norm_data[0] = c_norm_data[0] / ref_num;
      } else {
        // spatial_sum /= ref_spatial_dim * ref_num;
        c_norm_data[0] = c_norm_data[0] / ref_num;
        spatial_sum = c_norm_data[0];
        caffe_set<Dtype>(ref_channels, spatial_sum/2, this->blobs_[0]->mutable_cpu_data());
        // caffe_scal<Dtype>(ref_channels, Dtype(1.0 / ref_num), c_norm_data);
        // Dtype c_sum = caffe_cpu_asum<Dtype>(ref_channels, c_norm_data);
        // caffe_set<Dtype>(ref_channels, Dtype(c_sum / ref_channels), c_norm_data);
        // caffe_copy<Dtype>(ref_channels, c_norm_data, this->blobs_[0]->mutable_cpu_data());
        // LOG(INFO) << this->name();
        // for (int c = 0; c < ref_channels/50; ++c)
        //   std::cout << this->blobs_[0]->cpu_data()[c] << " ";
        // std::cout << std::endl;
      }
      first_bt_ = 0;
      return;
