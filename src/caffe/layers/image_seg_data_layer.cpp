...
void ImageSegDataLayer<Dtype>::load_batch_test(Batch<Dtype>* batch) {
  ...
  const float sca      = this->layer_param_.transform_param().scale_factors(0);
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    float ratio = sca;
    int sca_hei = std::floor(m_.rows * ratio);
    int sca_wid = std::floor(m_.cols * ratio);
    const int HEI = this->layer_param_.image_data_param().hei();
    const int WID = this->layer_param_.image_data_param().wid();
    if (sca_hei * sca_wid > HEI * WID) {
      ratio = ratio * std::sqrt(HEI*WID / (float)(sca_hei * sca_wid));
      LOG(INFO) << lines_[lines_id_].first << sca_hei << " " << sca_wid << " " << ratio << ": " << sca;
    }
    ...
    if (ratio != 1) {
      ...
      cv::resize(cv_img_ori, cv_img_resize, cv::Size(img_width, img_height), 0, 0, cv::INTER_LANCZOS4);
